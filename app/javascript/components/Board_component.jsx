...
function Card_component(props) {
  ...
  function deleteCard (){
    axios.delete(`http://127.0.0.1:3000/api/v1/cards/${props.cardId}`)
...
function Colomn_component(props) {
  ...
  function moveCardToLeftColumn(cardTitle, cardId){
    axios.put(`http://127.0.0.1:3000/api/v1/card_move_left/${cardId}`)
    .then(() => {props.moveCardToLeftColumnWithUpdate(cardTitle)})
  }

  function moveCardToRightColumn(cardTitle, cardId){
    axios.put(`http://127.0.0.1:3000/api/v1/card_move_right/${cardId}`)
  ...
  function handleKeyPress(e) {
    if (e.code == "Enter") {
      axios.post(`http://127.0.0.1:3000/api/v1/cards`,{ card: { title:  inputValue, colomn_id: props.columnId}})
      .then((response) => {
  ...
  function handleUpdateCard(code, value, cardId) {
    if (code == "Enter") {
      axios.put(`http://127.0.0.1:3000/api/v1/cards/${cardId}`, { card: { title:  value}})
      .then((response) => {
  ...
  useEffect(() => {
    axios.get(`http://127.0.0.1:3000/api/v1/cards/${props.columnId}`)
    .then((response) => {
...
export default function Board_component() {
  ...
  function deleteColumn (columnId){
    axios.delete(`http://127.0.0.1:3000/api/v1/colomns/${columnId}`)
      .then(() => {
  ...
  function moveCardToLeftColumnWithUpdate() {
    setColumns([])
    axios.get(`http://127.0.0.1:3000/api/v1/colomns/${board.id}`)
      .then((response) => {
  ...
  function handleKeyPressAddColumn(e) {
    ...
    if (e.code == "Enter") {
      axios.post(`http://127.0.0.1:3000/api/v1/colomns`,{ colomn: { title:  inputAddColumnValue, position: newColumnPosition, board_id: board.id}})
      .then((response) => {
        setColumnsCount(columnsCount + 1)
  ...
  useEffect(() => {
    axios.get(`http://127.0.0.1:3000/api/v1/boards/${link}`)
    .then((response) => {
